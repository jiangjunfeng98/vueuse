...
function combineCallbacks<T = any>(combination: Combination, ...callbacks: (((ctx: T) => void | Partial<T> | Promise<void | Partial<T>>) | undefined)[]) {
  if (combination === 'overwrite') {
    // use last callback
    return async (ctx: T) => {
      const effectiveCallbacks = callbacks.filter(callback => !!callback)
      if(effectiveCallbacks.lenth < 1) return ctx
      await effectiveCallbacks[effectiveCallbacks.length - 1](ctx)
      return ctx
    }
  }
  else {
